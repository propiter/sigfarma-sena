...
export function BajasInventario() {
  ...
  const loteFromState = location.state?.lote ?? null;

  const [activeTab, setActiveTab] = useState(loteFromState ? 'create' : 'list');
  const [selectedLote, setSelectedLote] = useState<any>(loteFromState);
  ...
  const handleSubmitBaja = async (data: any) => {
    try {
      ...
      if (response.ok) {
        showSuccessMessage('Solicitud de baja registrada exitosamente');
      ...
      showErrorMessage(error.message || 'Error al registrar la baja');
    }
  };
